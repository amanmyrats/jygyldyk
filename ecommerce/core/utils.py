...
def slugify(value):
    ...
    The SLUG_FUNCTION can be set with a dotted path to the slug
    function to use, defaults to 'oscar.core.utils.default_slugifier'.

    SLUG_MAP can be set of a dictionary of target:replacement pairs

    SLUG_BLACKLIST can be set to a iterable of words to remove after
    ...
    slugifier = import_string(settings.SLUG_FUNCTION)
    slug = slugifier(value, allow_unicode=settings.SLUG_ALLOW_UNICODE)
...
def format_datetime(dt, format=None):
    ...
    if is_naive(dt):
        ...
        logging.warning(
            "core.utils.format_datetime received native datetime")
    else:
