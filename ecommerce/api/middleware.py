...
from api.utils.request import get_domain
from api.utils.session import session_id_from_parsed_session_uri, get_session
from api import models

from webapps.basket.middleware import BasketMiddleware
from .utils.request import IsApiRequest
...
class ApiBasketMiddleWare(BasketMiddleware, IsApiRequest):
    ...
    api can not do that, because we don't want to put the burden
    of managing a cookie jar on api clients that are not websites.
    """
    ...
    def __call__(self, request):
        if self.is_api_request(request):
            ...
            # session baskets, since api uses only baskets from the
            # session.
    ...
    def process_response(self, request, response):
        if (
            ...
            # could have been created by api.
            cookie_key = self.get_cookie_key(request)
